...
def build_sim(env: Environment, start, goal, visualize: bool = True) -> tuple[Simulation, "Visualizer | None"]:
    ...
    known_map = KnownMap(
        ...
        world_cfg.y_min,
        world_cfg.y_max,
        world_cfg.grid_resolution,
        dynamic_memory_steps=sim_cfg.dynamic_memory_steps,
    )
    ...
    rrt = RRTPlanner(
        step=planner_cfg.rrt_step,
        max_iter=planner_cfg.rrt_max_iter,
        goal_sample_rate=planner_cfg.rrt_goal_sample_rate,
        allow_unknown=planner_cfg.rrt_allow_unknown,
        rewire_radius=planner_cfg.rrt_rewire_radius,
    )
    ...
    if visualize:
        ...
        viz = Visualizer(env, uav.body_radius, sim_cfg.safety_margin, uav.max_speed)
    return sim, viz
...
def run_scenario(name: str, visualize: bool = True) -> Metrics:
    ...
    elif name in {"dynamic", "urban"}:
        env, start, goal = scenarios.dynamic_crossing(world_cfg)
    else:
        raise ValueError(f"Unknown scenario: {name}")

    sim, viz = build_sim(env, start, goal, visualize=visualize)
    metrics = sim.run(start, goal, visualize=viz)
    if viz is not None:
        viz.show()
    return metrics
...
def main() -> None:
    parser = argparse.ArgumentParser(description="2D UAV path planning simulation with dynamic obstacles and adaptive velocity")
    parser.add_argument("--scenario", choices=["static", "dynamic", "urban"], default="dynamic")
